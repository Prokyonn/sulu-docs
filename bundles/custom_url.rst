...
different schema than the other URLs that will be generated by the system. For
example, the localization is not present in these URLs.

A custom URL consists of the following properties:
...
.. note::
    ...
    be marked that the target is missing, and will return a 404 on the website.
...
.. note::
    ...
    Be aware that different subdomains or different domains have to be
    configured on the web server. It is very important that you configure a
    wildcard for the subdomain if you will use it for custom URLs.
