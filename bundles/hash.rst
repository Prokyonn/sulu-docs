...
that takes the request, the object, and the identifier of an object (since the
...
with a newly created hash from the passed object, and it will throw an
...
Exception and returns a response with the status code `409 (CONFLICT)`. It
...
allows for overriding the object even if it has changed in the meantime.
