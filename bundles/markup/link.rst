...
The sulu-link tag allows linking to pages and other entities in the application by their ID.
The ID in the tag will be validated and replaced by a proper anchor tag when a response is generated.
Furthermore, the sulu-link tag allows adding a query string (?) or a hash (#) that is forwarded
...
There might be situations where you need to know which provider was used even after the tag was converted to an anchor tag. In that case you can use the provider attributes feature to pass provider information as a data attribute to the rendered anchor tag.
...
Now you will be able to target the links in CSS and Javascript by using the data attribute as a selector. This allows for:
...
If a Twig function were used to generate the links, each call would trigger a
...
If you want to completely remove the link text as well you can add the ``remove-if-not-exists`` attribute:

.. code-block:: html

    <sulu-link provider="page" href="123-123-123" remove-if-not-exists="true">Link Text</sulu-link>

    <!-- If the page does not exist, it will render nothing -->
