...
The ReferenceBundle tracks references among entities within the application.
It enables developers and maintainers to quickly determine the relationships between entities and understand how and where an entity is used. Presently, the ReferenceBundle is capable of monitoring the usage of Snippets and
Media within content entities such as `pages` and `snippets`. These references are managed separately for the draft
...
Content maintainers can see the references to a specific entity in the `Insights` tab of an entity like `Snippet`.
...
References update automatically upon saving an entity. You can manually update the
references by executing the `bin/console sulu:reference:refresh` command. This command optionally accepts a
...
.. note::
    ...
    Note that references are refreshed only for the current context. To refresh the references for both the
