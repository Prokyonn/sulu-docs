...
The RouteBundle provides the feature of managing website routes for custom entities.
These routes will be generated by calling the
...
schema and maintain "history" (301 redirects) routes for the old routes.

To resolve the route on request, you must provide a
...
This example will create a simple example entity that will be available
on the website with its own route.
...
After that, the entity is ready to get a route. To create a route for new entities,
simply call the `save` method of the service `sulu_route.manager.route_manager`.
...
As there is no cascading on the database layer, you must call the `remove` method on the
