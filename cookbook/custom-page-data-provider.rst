...
comes with a few included providers and also allows you to implement new ones to load custom entities.
...
.. code-block:: php
    ...
    class AuthorPageQueryBuilder extends QueryBuilder
    {
        ...
        {
            // reuse the existing functionality of the default QueryBuilder implementation
            ...
            // add a custom condition to check the author of the page to the query
            $sql2Where[] = "(page.[i18n:" . $locale . "-author] = " . $author->getId() . ")";

            // optional: add a custom condition to check the value of a specific property of the page to the query
...
**2. Register a** ``PageDataProvider`` **service that uses a custom** ``QueryBuilder`` **implementation**
...
**3. Use the registered** ``author_pages`` **DataProvider in the template**
...
You can implement custom sorting criteria by creating a new ``DataProvider`` class that extends the ``PageDataProvider``
class and using it instead. That way, you can sort by a property that isn't provided by the ``PageDataProvider``.
When there is a lot of business logic, it is probably better to create a custom entity instead.
...
.. list-table::
    ...
    * - `setView`
      ...
      - Defines where the deep link should navigate to when clicking on a smart content item.

**2. Use the custom** ``DataProvider`` **in** ``services.yaml`` **.**
...
All other parameters remain the same as when you would register a ``PageDataProvider`` service that uses a custom ``QueryBuilder`` implementation.
