...
The Sulu Document Manager is a layer that sits between the PHPCR repository
...
It is similar in concept to a typical ORM (for example, Doctrine ORM), but with
some differences.
...
- ``persist`` commits the changes to the *node* immediately; changes made to the
  ...
  to think of `persist()` as a function that prepares a `snapshot` of the
...
- Documents are defined with "Behavior" interfaces that the event
