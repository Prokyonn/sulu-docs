...
Displays a list with the ability to assign a single asset from the media section
to a page. It also allows you to define a position, which can be handled later in the
template.
...
.. list-table::
    ...
    * - ``displayOptions``
      - collection
      - A collection of booleans that defines which positions assets
        can be assigned to (``leftTop``, ``top``, ``rightTop``, ...).
    * - ``defaultDisplayOption``
      - string
      - Defines which of the ``displayOptions`` is the default.
...
function to control which `disposition header`_ the target URL should use:
