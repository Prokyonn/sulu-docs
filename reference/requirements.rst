...
your system must fulfill to run a Sulu application.
...
* `PHP`_ 8.2 or higher
* The ``dom`` and ``xml`` extensions for PHP
* The ``gd``, ``imagick``, or ``vips`` extensions for PHP
* The ``pdo_sqlite`` extension when using `Loupe`_ as the `SEAL`_ Search Engine Adapter
...
If you want to build parts of the system yourself, you will additionally need:
